...
| Token theft | Read auth.json | HIGH — full access | LOW (requires same user) | chmod 600. The hub and desktop apply the same `auth.json` file rule: they refuse a world-readable file and warn but accept a group-readable one. The desktop still uses its descriptor walk, and still refuses anything that is not a regular file, but for `auth.json` alone it drops two checks it applies elsewhere: the parent need not be owner-only, and the file need not be owned by the current user. Both exist so the desktop accepts every credential the hub does — the hub reads the file's mode and nothing else, so a group-writable directory or a root-owned file works there and would otherwise fail here. What that admits: a member of a group that can write the directory may replace or delete the file. On a directory that permissive they can already read a 0640 token, and a token they plant is rejected by the hub that holds the real one, so the exposure is denial of service rather than a credential they did not have. `runtime.json`, the pinned-key store and the TLS key keep both checks. |
