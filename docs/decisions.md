...
- Releases are draft-until-built, and a published one is refused: `create-release` exits before any cleanup or build when the tag already has a published release (#247). Reusing one meant every later job wrote to something users could see, and `gh release upload --clobber` replaces one asset at a time, so a failure halfway left the public release holding a mixture of old and new assets. The refusal is a snapshot taken when `create-release` runs: a writer who publishes the admitted draft mid-run puts the later `--clobber` uploads back on a public release, which this cannot prevent. Rebuilding a published release means cutting a new version, or converting it back to a draft — and that second path takes it out of public view at once and deletes its `*-agent-*` assets only, so a failing rebuild leaves a draft holding the old hub, desktop and checksum assets plus whichever new uploads finished. Delete any other draft for the tag first, since the cleanup keeps whichever release the API lists first (#270). A run that published the release and then reported failure also lands on the refusal; the release is complete and wants checking, not re-running. release-please creates a DRAFT release (`draft: true`), assets attach to it, and `publish-release` flips it public only when every build succeeded. `force-tag-creation: true` is required with drafts: GitHub defers tag creation for draft releases, which would otherwise break release-please's next version computation.
...
- `contents: write` is granted per job rather than at workflow level: `matrix` gets `contents: read`, and the six jobs that call `gh release create`, `gh release upload`, `gh api -X DELETE` or `gh release edit` get write. The workflow declares `permissions: {}` as its baseline, and GitHub sets every permission a map does not list to `none`, so a job added without its own map gets nothing rather than the repository default. The four build jobs still hold write only because they attach their artefacts to the release itself; moving those to `actions/upload-artifact` (#260, #247) is what removes it. Every checkout sets `persist-credentials: false`, so build code cannot recover the checkout credential; `create-release` and `publish-release` run `gh auth setup-git` before their `git ls-remote`, because that read is otherwise anonymous and would fail if the repository became private. It can still be handed the job token by an action's own default input (#280).
