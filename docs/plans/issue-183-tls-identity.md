...
| P9 the relay cannot buffer without bound | **delivered in both directions.** Responses stream under a single outstanding credit per relay; requests, multipart included, cross in 256 KiB pieces through the upload pipe rather than being materialised. Eight relays may be in flight, four of them uploads, and a request past that is refused rather than queued. Every wait ends: an unanswered acknowledgement or a stalled upload expires within the request timeout and releases its thread, response, socket and map entry. The WebSocket's own TLS connect still has no explicit deadline — its own follow-up |
